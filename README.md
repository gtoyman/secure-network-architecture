...
Actually you need this setup twice. One gateway with vpn is used as DNS server for all dns requests, another gateway with vpn is used as gateway for the rest of the traffic.  That way they can't return to your system based on dns requests. The second gateway is ip-based only.
